...
function AssignBox({
  ...
  const empty = !client.trim() && !project.trim() && !campaign.trim();
  return (
    <div className="assign-box" aria-label="Assign mission">
      ...
      <button
        disabled={empty}
        onClick={() => onAssign(missionId, { client: client || undefined, project: project || undefined, campaign: campaign || undefined })}
      >
        Assign
      </button>
      <button className="ghost" onClick={() => onAssign(missionId, { clear: true })}>Clear</button>
    </div>
  );
}
...
export default function TaxonomyBrowser({
  ...
  return (
    <div className="taxonomy-browser">
      <div className="taxonomy-tree" aria-label="Taxonomy groups">
        ...
        {campaigns.length > 0 && (
          <details>
            ...
            {campaigns.map((c) => (
              <button
                key={`${c.client}/${c.project}/${c.campaign}`}
                className="taxonomy-campaign"
                onClick={() => onFilter({ client: c.client, project: c.project, campaign: c.campaign })}
              >
      ...
      {selectedId && <AssignBox key={selectedId} missionId={selectedId} onAssign={onAssign} />}
    </div>
  );
}
