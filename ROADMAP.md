...
- ✅ **`agency_studio/engines/local_media.py`** — warm single-resident `ModelManager`.
  ...
  - **image** — a **selectable** mflux model (registry in `models.py`, GUI dropdown):
    **FLUX.1-schnell** (default, 8-bit mirror), **Z-Image-Turbo** (fast, 8-step),
    **FLUX.2-klein-4B** (modern) — all non-gated, Apache-2.0, 16 GB-friendly; the loader
    is backend-pluggable for a future non-mflux entry. **STT** — Whisper large-v3-turbo
